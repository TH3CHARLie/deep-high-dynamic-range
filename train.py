...
class CustomLossFunction(tf.keras.losses.Loss):
    def call(self, y_true, y_pred):
        return tf.reduce_sum(tf.square(range_compress(y_true) - range_compress(y_true))))
...
def train_main(config: Config):
    ...
    if not os.pash.exists('./tensorboard'):
        pathlib.Path('./tensorboard').mkdir(parents=True, exist_ok=True)
